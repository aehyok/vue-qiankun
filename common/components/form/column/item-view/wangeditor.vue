...
const emit = defineEmits(["dochange"]) // 先申明
...
const editorContent = computed({
  ...
  set(value) {
    updateval(value);
    emit("dochange", value, state.editor.txt.text());
  },
});
...
onMounted(() => {
  ...
  editorConfig.onchange = (html) => {
    // editorContent = html
    emit("dochange", html, state.editor.txt.text());
  };
  state.editor.create();
  state.editor.txt.html(props.data);
});
watch(props.data, (newV, oldVal) => {
  if (newV !== state.editor.txt.html()) {
    if (props.canAssignHtml) {
      state.editor.txt.html(newV);
    }
    updateval(newV);
    emit("dochange", newV, state.editor.txt.text());
  }
});
</script>
