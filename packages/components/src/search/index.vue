<template>
  <template v-for="(item, index) in searchParameters" :key="index">
    <component :is="item.type + 'Search'" :column="item" v-model:data="item.value" />
    <!-- 输入框 -->
...
const reset = () => {
  ...
  emit('search', state.keyword, state.getDefault, state.publishDate, state.startDate, state.endDate)
}
